...
try: 
    import PyPolyChord
    polychord_import = True
except:
    polychord_import = False

if polychord_import:
    from PyPolyChord.settings import PolyChordSettings
    from PyPolyChord.priors import UniformPrior as PolyUniformPrior
...
class fitting(object):
    ...
    def __init__(self, forwardmodel):
        ...
        self.dir_polychord = os.path.join(self.params.out_path,'polychord')

        if self.MPIrank == 0:
            folders = [self.params.out_path, self.dir_mcmc, self.dir_multinest,self.dir_polychord]
            for f in folders:
...
##########
    def polychord_fit(self, resume=None):

        logging.info('Start POLYCHORD fit')

        if resume is None:
            resume = self.params.nest_poly_resume

        if resume:
            # Resuming from previous run. Restart from scratch if number of live points has changed.
            filename = os.path.join(self.dir_polychord, '1-live.points')
            if os.path.isfile(filename):
                livepoints = sum(1 for line in open(filename))
                if livepoints != self.params.nest_nlive:
                    # todo this should only be done if MPIrank = 0
                    logging.warning('Cannot resume previous POLYCHORD run, the number of live points has changed')
                    logging.warning('Removing previous POLYCHORD chains')
                    for file in os.listdir(self.dir_polychord):
                        file_path = os.path.join(self.dir_polychord, file)
                        try:
                            if os.path.isfile(file_path):
                                os.unlink(file_path)
                        except:
        ...
        def polychord_loglike(cube):
            ...
            fit_params_container = asarray([cube[i] for i in range(len(self.fit_params))])
            chi_t = self.chisq_trans(fit_params_container, data, datastd)
            logL = (-1.)*np.sum(np.log(datastd*np.sqrt(2*np.pi))) - 0.5 * chi_t
#             sigma = np.mean(datastd)
            
#             logL = -log(2*pi*sigma*sigma)*ndim/2.0
#             logL += -chi_t/2/sigma/sigma
            
            
            
            phi = [0.0] * nderived
#             phi[0] = sqrt(chi_t)
#             print(chi_t,logL,phi)
#             print(phi)
            
            return logL,phi

#         def polychord_uniform_prior(cube, ndim, nparams):
#             # prior distributions called by multinest. Implements a uniform prior
#             # converting parameters from normalised grid to uniform prior
#             for i in range(len(self.fit_params)):
#                 cube[i] = (cube[i] * (self.fit_bounds[i][1]-self.fit_bounds[i][0])) + self.fit_bounds[i][0]
...
#                 theta[i] = PolyUniformPrior(-1, 1)(x)
#                 print(theta)
                
            for i in range(len(self.fit_params)):
                cube[i] = (hypercube[i] * (self.fit_bounds[i][1]-self.fit_bounds[i][0])) + self.fit_bounds[i][0]
#             print('hyper',hypercube)
#             print(cube)
            return cube
        ...
        settings.file_root = self.params.nest_poly_file_root
        settings.base_dir  = self.dir_polychord
        settings.read_resume = self.params.nest_poly_resume
        settings.do_clustering = self.params.nest_poly_clustering
        settings.nlive     = ndim * 25
        settings.num_repeats = ndim * 5
        settings.precision_criterion = self.params.nest_poly_precision
        
        

        logging.info('Polychord output dir is %s' % self.dir_polychord)


        PyPolyChord.run_polychord(polychord_loglike,ndim,nderived,settings,polychord_uniform_prior)
...
#                         multimodal=self.params.nest_multimodes,
#                         max_modes=self.params.nest_max_modes,
#                         outputfiles_basename=os.path.join(self.dir_polychord, '1-'),
#                         const_efficiency_mode = self.params.nest_const_eff,
#                         importance_nested_sampling = self.params.nest_imp_sampling,
#                         resume = resume,
#                         verbose = self.params.nest_verbose,
