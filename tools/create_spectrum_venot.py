...
class convert_venot(object):
    def __init__(self,params,chem_profile,tp_profile): 
        
        self.params = params
        self.chem_profile_file = chem_profile
        self.tp_profile_file   = tp_profile
    ...
    def load_venot_model(self):
            ...
            apt = np.loadtxt(self.tp_profile_file)
...
#             pl.plot(self.ven_temperature, np.log(self.ven_pressure))
            ...
            #determining cross section naming convention
            xseclist = glob.glob(self.params.in_xsec_path+'/*')
            namestem = xseclist[0].split('/')[-1]
            namestem2 = namestem.split('.',1)[-1]
    

            for mol_idx, mol_val in enumerate(self.ven_molecules):

                molpath1 = os.path.join(self.params.in_xsec_path, '{0}.{1}'.format(mol_val,namestem2))
...
#             gases = ['H2','N2']
    ...
    def modify_params(self,params,pressure_flag=True):
        '''
        ...
        if pressure_flag is True:
            params.atm_max_pres = np.max(self.ven_pressure)
            params.atm_min_pres = np.min(self.ven_pressure)
            logging.info('Using pressure bounds in input file')
        else:
            logging.info('Using pressure bounds in paramter file')
        params.mode = 'custom'
        return params
    
    
    def set_venot_profiles(self,csob):
        '''
        ...
        csob.atmosphereob.inactive_gases = ['H2', 'HE', 'N2']
...
#         exit()
    ...
    def get_tp_profile(self,pressure):
        ''' 
        takes taurex initialised tp_profile and adapts venot tp_profile
        ''' 
    ...
    def get_chemical_profile(self,pressure_profile):
        '''
        takes taurex initialised chemical profile and adapts it with venot tp_profile
        '''
        self.nlayers = self.params.atm_nlayers
        active_mixratio_profile = np.zeros((len(self.ven_active_gases), self.nlayers))
        inactive_mixratio_profile = np.zeros((3, self.nlayers)) #always H2, He, N2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tp_profile',
                        dest='tpprofile',
                        default='tp.dat',
                        help='Input temp-pressure profile file')

    parser.add_argument('--infile_pbounds',
                        dest='ven_pressure',
                        default=True,
                        help='takes pressure upper and lower bounds from input file. Set False to use parameter file bounds')
    ...
    venotob = convert_venot(params=params,chem_profile=options.chemprofile,tp_profile=options.tpprofile)
    
    #partially initialising standard create_spectrum instance 
    spectrumob = create_spectrum(params=venotob.modify_params(params,pressure_flag=options.ven_pressure), nthreads=options.nthreads,full_init=False)
#     spectrumob = create_spectrum(params=params, nthreads=options.nthreads,full_init=False)
    ...
    spectrumob = venotob.set_venot_profiles(spectrumob)
