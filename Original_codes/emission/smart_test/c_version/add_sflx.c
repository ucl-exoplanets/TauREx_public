/* add_sflx.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int add_sflx__(integer *nlyr, doublereal *tl, doublereal *rl,
	 real *umu0nz, doublereal *dir, doublereal *fup, doublereal *fdn, 
	doublereal *urt, doublereal *drb, doublereal *uft, doublereal *dfb, 
	doublereal *ft, doublereal *fb, doublereal *flxu, doublereal *flxd)
{
    ...
    static integer nlev;
    static doublereal fupk, dnflx, upflx;


/* cccccccccccccccccccccccccc  a d d _ s f l x   ccccccccccccccccccccccccc */
/* c                                                                    cc */
/* c    p u r p o s e :                                                 cc */
/* c                                                                    cc */
/* c    this program uses the layer adding method for finding solar     cc */
...
/* L1201: */
    ...
    dd[nlev - 1] = 1. / (1. - rt[nlev - 1] * rl[nlev]);
    du[nlev - 1] = 1. / (1. - rl[nlev] * rb[nlev]);
...
/*       layers to the base of the existing inhomogeneous layer */
	...
	dnflx = dir[k] / *umu0nz + (ft[k] + rt[k - 1] * fupk) * dd[k - 1];
	fdn[k + 1] = dnflx * fdn[k + 1];
	ft[k + 1] = fdn[k + 1] + tl[k] * (ft[k] + rt[k - 1] * fupk) * dd[k - 
		1];
/* L1401: */
    }
...
/*       for combined layers.  start at bottom. */
    ...
    for (l = 1; l <= i__1; ++l) {
	k = nlev - l;
	upflx = dir[k + 1] + (fb[k + 1] + rb[k] * fdn[k + 1]) * du[k - 1];
	fup[k] = upflx * fup[k];
...
/*       note: these values use the scaled values of fdn, fup */
